...
public class Regex {
    ...
    private static final String TIME_REGEX = "\"receiveHeadersEnd\":[0-9]*.?[0-9]+";
    private static final String DOUBLE_REGEX = "\\d*\\.?\\d+";
    
    @Test
    public void testWordBoundry() {
        assertEquals(" 2ndUser/EC", Pattern.compile("User/" + "([a-zA-Z0-9])*").matcher(" 2ndUser/EC").group());
        assertEquals(" 2ndUser/EC", Pattern.compile("2ndUser/" + "([a-zA-Z0-9])*").matcher(" 2ndUser/EC").group());
        //assertEquals(" 2ndAcarsUser/23EC", pattern.matcher(" 2ndAcarsUser/23EC").group());
    }
    ...
    public static void main(String[] args) {
        ...
        extractStringUsingRegex(message, URL_REGEX);
        extractStringUsingRegex(message, TIME_REGEX);

        message = "330 330.2";
        extractStringUsingRegex(message, DOUBLE_REGEX);
    }

    public static void extractStringUsingRegex(String json, String regexPattern) {
